...
export class SimuladorComponent implements OnInit {
  ...
  func(event) {
    this.simForm.value.usagePercentage = Number(event.target.value);
    console.log(this.simForm.value.usagePercentage);
  ...
  selectExtraFromSelect(event) {
    this.showContainer = true;
    this.selectExtra = event.target.value;
    console.log(this.selectExtra);
    console.log(this.f.extrasWithTa);
    console.log(this.f.extrasWithoutTa);
  ...
  calculateTotalAnualCost() {
    ...
    for (let i = 0; i < this.simForm.value.extrasWithTa.length; i++) {
      ...
      this.simForm.value.anualTotalCost += (this.simForm.value.extrasWithTa[i].value * this.monthsInAYear) + (this.simForm.value.extrasWithTa[i].value * this.autonomousTributation * this.monthsInAYear);
    }
    for (let j = 0; j < this.simForm.value.extrasWithoutTa.length; j++) {
      console.log(j, this.simForm.value.extrasWithoutTa[j]);
      this.simForm.value.anualTotalCost += (this.simForm.value.extrasWithoutTa[j].value * this.monthsInAYear);
    }
  ...
  calculateMonthlyTotalCost() {
    ...
    console.log(this.simForm.value.monthlyTotalCost);
  }
  ...
  /* exportPDFforComercial() {
    ...
    let hInsurance = this.simForm.value.healthInsurance;
    let oBonus = this.simForm.value.otherBonus;
    let netSalaryWithD = this.simForm.value.netSalaryWithDuo;
    let netSalaryWithoutD = this.simForm.value.netSalaryWithoutDuo;
    let outrosExtras = this.simForm.value.extrasWithTa.concat(this.simForm.value.extrasWithoutTa);
    let margem = this.simForm.value.marginPercentage;
    let utilizacao = this.simForm.value.usagePercentage;
    let valorIRS = this.simForm.value.IRSValue;
    let custoAnual = this.simForm.value.anualTotalCost;
    let custoMensal = this.simForm.value.monthlyTotalCost;
    let custoDiario = this.simForm.value.dailyTotalCost;
    let custoHora = this.simForm.value.hourlyTotalCost;
    let rateAnual = this.simForm.value.anualRate;
    let rateMensal = this.simForm.value.monthlyRate;
    let rateDiario = this.simForm.value.dailyRate;
    let rateHora = this.simForm.value.hourlyRate;
    ...
    doc.save('Simulacão_detalhada ' + consultant + '.pdf');
  } */
}
